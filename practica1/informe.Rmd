...
Vemos que a primera vista parece que algunas categorias van a influir más en la respuesta que otras. Las que tinen boxplots muy similares entre categorías menos representativas van a ser, como es el caso de `Holiday` y `Working day`.
...
- **Normalidad de los residuos**: Desviación de la normal tanto en valores pequeños y granes, no se cumple.
...
## Transformación de la variable resupesta
...
## Modelo final - Transformación BoxCox
...
```{r messages = FALSE, fig.width=10, fig.height=10}
library('effects')
par(mfrow=c(2,4))
...
## Modelo 5: Transformación polinómica
