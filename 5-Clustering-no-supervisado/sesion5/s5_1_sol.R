...
## Escalamos para que todas las caracteristicas varien sobre el mismo rango.
## Atención! No hay que escalar los datos si estan tomados en la misma escala
wine2 <- scale(wine)
heatmap(as.matrix(wine2))
## Podemos observar zonas mas claras y zonas mas oscuars, lo que nos indica la separacion en distintos clusters
...
## Por norma general:
##  la distancia euclidean es la mas utilizada
##  el criterio ward el mas utilizado
## Lo que mas influye es el tipo de agrupacion
...
## Vemos que los mas claros son los que utilizamos el metodo de agrupacion ward y que claramente los separa en tres grupos 
...
## Uitlizamos 3 grupos, k=3
ct1 <- cutree(hc1,k=3)
ct2 <- cutree(hc2,k=3)
ct3 <- cutree(hc3,k=3)
ct4 <- cutree(hc4,k=3)

## Vemos en todos los resultados anteriores que parece que las observaciones en el conjunto de datos ya vienen ordenadas por los tipos que intentamos descubrir.
## Ya que estan en orden.
...
##similitud que hay entre las distancias originales y las distancias del dendograma son elevadas, significa que el arbol esta bien construido.
...
## En las que los boxplots difieren mas entre ellos, (alcohol, totalPhenols, flavanoids)

pca <- PCA(wine,graph=FALSE)
par(mfrow=c(1,2))
plot(pca,col.ind=ct1,label='none')
plot(pca,choix = "var",cex=0.7)
## TODO: interpretar grafico
...
# numero ideal = 5 para explicar mas del 0.5 %
